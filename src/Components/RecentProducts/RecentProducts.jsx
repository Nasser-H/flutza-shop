      ...
      {isLoading ? <Loading />
        ...
        <div className="flex flex-wrap gap-5 justify-center py-6">
          {data?.data.data.filter(item => item?.title.toLowerCase().includes(searchProducts?.search?.toLowerCase())).map((product, index) =>
            <div key={index} className="lg:w-1/6 sm:w-1/4 w-10/12 bg-white border border-transparent hover:border-main shadow-md relative group/wishList group/image overflow-hidden cursor-pointer">
